...
get_percent()
{
  case $(uname -s) in
    Linux)
			percent=$(LC_NUMERIC=en_US.UTF-8 top -bn2 -d 0.01 | grep "Cpu(s)" | tail -1 | sed "s/.*, *\([0-9.]*\)%* id.*/\1/" | awk '{print 100 - $1}' | xargs printf "%.0f%%")
...
main() {
  ...
  if [ "$cpu_load" = true ]; then
    ...
    cpu_value_without_percent=$(echo "$cpu_percent" | sed 's/%//')
    # cpu_value_without_percent=90
    if [[ $cpu_value_without_percent -ge 75 ]]; then
