...
main()
{
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${yellow}],} ${left_icon} #[fg=${light_purple},bg=${bg_color}]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    powerbg=${bg_color}
  else
